...
#
# Tags look like `v<Woodpecker CI agent version>-<release>`:
#
# - if defaults/main.yml points at a Woodpecker CI agent version that has never
#   been released, the release counter restarts at 0 (`v3.17.0-0`)
...
version="$(sed -nE 's|^woodpecker_ci_agent_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Woodpecker CI agent version from $defaults_path"
	exit 1
fi
